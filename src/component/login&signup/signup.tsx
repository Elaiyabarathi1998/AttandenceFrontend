...
import outletadmin from './out2.gif';
...
export const Signup: React.FC = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'email') {
      ...
      // Username validation: Between 6 and 16 characters
      const usernameRegex = /^.{6,16}$/;
      setName(value);
      setUsernameError(usernameRegex.test(value) ? null : 'Username must be 6 to 16 characters');
    } else if (name === 'password') {
      // Strong password policy: At least 8 characters, including uppercase, lowercase, and numbers
      const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/;
      setPassword(value);
      setPasswordError(passwordRegex.test(value) ? null : 'Password is too weak');
    }
  };
